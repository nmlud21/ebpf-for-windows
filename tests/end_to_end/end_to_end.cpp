...
/**
 * @brief Comprehensive test that validates the complete proof of verification flow for a native eBPF module.
 * 
 * This test validates all 5 steps of the proof of verification system using the production-signed
 * droppacket.sys native eBPF kernel driver:
 * 1. User-mode calls eBPF service to authorize module (ebpf_authorize_native_module_wrapper)
 * 2. Service validates digital signature with WinVerifyTrust (certificate and EKUs)
 * 3. Service authorizes module with kernel (module_id + hash + timestamp)
 * 4. Module loads and attaches via NMR (Network Module Registrar)
 * 5. Kernel validates hash at attach time and enforces authorization
 * 
 * The test ensures:
 * - Signature validation works on production-signed binary (Step 2)
 * - Authorization is properly stored in kernel (Step 3)
 * - Module successfully loads with valid authorization (Steps 4-5)
 * - Complete proof of verification chain passes for droppacket.sys
 * 
 * IMPORTANT: This test uses droppacket.sys, a production-signed native eBPF binary with:
 ...
 * 
 * The test will be skipped if test signing is enabled, as it requires real production signing.
 */
TEST_CASE("proof_of_verification_positive", "[end-to-end]")
{
    _test_helper_end_to_end test_helper;
    test_helper.initialize();

    // Skip this test if test signing is enabled - we need production signing
    bool test_signing_enabled = false;
    bool hvci_enabled = false;
    REQUIRE(ebpf_get_code_integrity_state(&test_signing_enabled, &hvci_enabled) == EBPF_SUCCESS);
    
    // if (test_signing_enabled) {
    //     SKIP("Test requires production-signed eBPF binary (test signing is enabled)");
    // }
    ...
    // STEP 1 & 2: Authorize native module
    // This triggers:
    // - Signature validation (WinVerifyTrust) - validates droppacket.sys is production-signed
    // - Certificate issuer check ("Microsoft Corporation eBPF Verification")
    // - EKU validation (Code Signing + eBPF Verification)
    // - Root CA validation (Microsoft Root CA 2011 thumbprint)
    // - Hash computation (SHA256 of droppacket.sys)
    // - Authorization storage in kernel with module_id and timestamp
    //
    // droppacket.sys is a production-signed native eBPF kernel driver that has:
    // - Certificate issued by "Microsoft Corporation eBPF Verification" CA
    // - Both Code Signing (1.3.6.1.5.5.7.3.3) and eBPF Verification (1.3.6.1.4.1.311.133.1) EKUs
    // - Valid signature chain to Microsoft Root CA 2011
    REQUIRE(ebpf_authorize_native_module_wrapper(&provider_module_id, "droppacket.sys") == EBPF_SUCCESS);

    // STEP 3: Create service for the native driver
    _create_service_helper(
        L"droppacket.sys", NATIVE_DRIVER_SERVICE_NAME, &provider_module_id, &service_handle);

    // STEP 4: Load native module via IOCTL
    // This triggers NMR (Network Module Registrar) attach in kernel
    service_path = service_path + NATIVE_DRIVER_SERVICE_NAME;
    REQUIRE(
        test_ioctl_load_native_module(
            service_path,
    ...
    // The successful load proves kernel validation passed during NMR attach:
    // - Kernel looked up authorization by module_id
    // - Computed SHA256 hash of the actual file
    // - Compared authorized_hash with actual_hash
    // - Deleted the authorization (one-time use)
    // - Allowed attachment because hashes matched
    REQUIRE(*module_handle.get_handle_pointer() != ebpf_handle_invalid);
    REQUIRE(count_of_programs > 0);

    // Delete the service
    Platform::_delete_service(service_handle);

    // TEST PASSED: Complete proof of verification flow succeeded!
    // The native module:
    // ✓ Had a valid digital signature with correct certificate
    // ✓ Certificate was issued by "Microsoft Corporation eBPF Verification"
    // ✓ Certificate had both Code Signing and eBPF Verification EKUs
    // ✓ Was authorized with kernel (module_id + hash + timestamp)
    // ✓ Passed kernel-side hash validation at attach time
    // ✓ Authorization was consumed (one-time use)
    // ✓ Module loaded successfully and is ready for use
}
